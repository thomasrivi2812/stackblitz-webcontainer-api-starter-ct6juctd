...
export const PERSONAS: Persona[] = [
  {
    ...
    enjeux: [
      { icon: 'lock', titre: 'Souveraineté des données', texte: 'Vos données sensibles ne peuvent pas transiter hors de France ni dépendre du Cloud Act.' },
      { icon: 'shield', titre: 'Conformité réglementaire', texte: 'ISO 27001, HDS, RGPD — vous devez justifier vos choix d’hébergement à votre RSSI et à la direction.' },
      { icon: 'bolt', titre: 'Disponibilité critique', texte: 'Vous avez besoin d’un SLA > 99,9 % et d’une architecture redondante N+1.' },
      { icon: 'euro', titre: 'Maîtrise des coûts IT', texte: 'Vos factures cloud sont imprévisibles. Vous cherchez une alternative à coût fixe pour les charges stables.' },
    ],
    problemesTitre: 'Les problèmes que nous résolvons pour les DSI',
    problemes: [
      { icon: 'globe', titre: 'Dépendance aux hyperscalers étrangers', texte: 'AWS, Azure, GCP sont soumis au Cloud Act américain. Vos données critiques peuvent être réclamées par une juridiction étrangère sans que vous en soyez informé.' },
      { icon: 'growth', titre: 'Coûts cloud imprévisibles', texte: 'Le « bill shock » en fin de mois est devenu un sujet de direction. Pour les charges stables, le cloud public coûte structurellement plus cher que la colocation.' },
      { icon: 'building', titre: 'Salle serveur interne à gérer', texte: 'Climatisation, électricité redondante, sécurité physique — maintenir une salle serveur interne mobilise des ressources disproportionnées.' },
      { icon: 'ruler', titre: 'Audits de conformité chronophages', texte: 'ISO 27001, HDS, PCI-DSS — documenter votre chaîne d’hébergement prend du temps. Un prestataire certifié transfère une partie de cette charge.' },
      { icon: 'wrench', titre: 'Interventions physiques urgentes', texte: 'Quand un serveur tombe à 2h du matin, vous avez besoin d’un accès immédiat ou d’une équipe capable d’intervenir à votre place.' },
      { icon: 'network', titre: 'Connectivité multi-opérateur', texte: 'Votre connectivité ne peut pas dépendre d’un seul fournisseur. Vous devez pouvoir choisir et faire concurrencer vos opérateurs.' },
    ],
  ...
  },
  {
    ...
    enjeux: [
      { icon: 'chart', titre: 'Budget IT imprévisible', texte: 'Chaque mois, la facture cloud change. Difficile de budgétiser et de présenter un prévisionnel à la direction.' },
      { icon: 'home', titre: 'Salle serveur à gérer', texte: 'Vous avez un placard serveur qui chauffe, dont personne ne veut s’occuper et qui vous inquiète la nuit.' },
      { icon: 'handshake', titre: 'Prestataire injoignable', texte: 'Chez les grands opérateurs, vous êtes un ticket dans une file. Vous avez besoin d’un vrai interlocuteur.' },
      { icon: 'mapPin', titre: 'Proximité indispensable', texte: 'Vos équipes doivent pouvoir intervenir physiquement dans la journée si besoin.' },
    ],
    problemesTitre: 'L’IT des PME : des contraintes bien réelles',
    problemes: [
      { icon: 'euro', titre: 'Factures cloud qui augmentent chaque année', texte: 'Les prix des hyperscalers augmentent régulièrement. Pour des charges stables, vous payez un premium de flexibilité dont vous n’avez pas besoin.' },
      { icon: 'wrench', titre: 'Salle serveur interne ingérable', texte: 'Climatisation en panne, coupure électrique, intrusion — la gestion d’une salle serveur interne expose votre activité à des risques disproportionnés.' },
      { icon: 'document', titre: 'Exigences clients et assureurs', texte: 'Vos clients grands comptes ou votre assurance cyber exigent des garanties sur la localisation et la sécurité de vos données.' },
      { icon: 'user', titre: 'Manque d’interlocuteur dédié', texte: 'Chez les grands opérateurs, vous naviguez entre des portails et des tickets. Personne ne connaît votre contexte.' },
    ],
  ...
  },
  {
    ...
    enjeux: [
      { icon: 'institution', titre: 'Doctrine Cloud au centre', texte: 'La DINUM recommande des hébergements qualifiés. Vous devez justifier vos choix devant vos instances de contrôle.' },
      { icon: 'lock', titre: 'Données sensibles hors Cloud Act', texte: 'Vos données à caractère personnel ou stratégique ne peuvent pas dépendre de législations extraterritoriales.' },
      { icon: 'leaf', titre: 'Engagements RSE et bilan GES', texte: 'Votre collectivité a des objectifs de réduction carbone. Votre IT doit contribuer, pas alourdir le bilan.' },
      { icon: 'document', titre: 'Commande publique', texte: 'Vos marchés ont des contraintes contractuelles spécifiques. Vous avez besoin d’un prestataire habitué.' },
    ],
    problemesTitre: 'Les défis IT spécifiques du secteur public',
    problemes: [
      { icon: 'scale', titre: 'Conformité à la doctrine Cloud au centre', texte: 'La DINUM recommande d’héberger les données sensibles sur des offres qualifiées ou des infrastructures souveraines françaises.' },
      { icon: 'globe', titre: 'Risque d’extraterritorialité', texte: 'Les solutions cloud américaines sont soumises au Cloud Act. Vos données peuvent être réclamées par une juridiction étrangère.' },
      { icon: 'chart', titre: 'Obligation de reporting RSE', texte: 'Votre bilan GES et vos objectifs de neutralité carbone incluent le numérique, part croissante de l’empreinte des collectivités.' },
      { icon: 'search', titre: 'Justification devant les instances', texte: 'Chambre régionale des comptes, CNIL, préfecture — vos choix peuvent être questionnés. Documentation solide nécessaire.' },
    ],
  ...
  },
  {
    ...
    enjeux: [
      { icon: 'signal', titre: 'Extension de PoP', texte: 'Vous cherchez à déployer votre présence réseau sur Rennes, Rouen ou Paris sans construire votre propre site.' },
      { icon: 'plug', titre: 'Meet-me-room neutre', texte: 'Vous avez besoin d’un site pour vous interconnecter avec d’autres opérateurs ou acteurs cloud sans contrainte.' },
      { icon: 'flame', titre: 'Haute densité', texte: 'Vos équipements GPU ou HPC nécessitent un refroidissement liquide que la plupart des sites ne peuvent pas accueillir.' },
      { icon: 'growth', titre: 'Déploiement national', texte: 'Vous cherchez un partenaire capable de vous suivre sur plusieurs sites en France, pas un prestataire mono-site.' },
    ],
    problemesTitre: 'Les enjeux d’infrastructure des opérateurs en France',
    problemes: [
      { icon: 'map', titre: 'Extension sur les métropoles régionales', texte: 'Paris est saturé. Rennes, Rouen, Lyon, Bordeaux deviennent stratégiques pour la latence et la couverture.' },
      { icon: 'bolt', titre: 'Besoins haute densité non satisfaits', texte: 'Vos équipements GPU, accélérateurs IA ou HPC consomment 20-50 kW par baie. Peu de datacenters peuvent les accueillir.' },
      { icon: 'handshake', titre: 'Interconnexion avec l’écosystème', texte: 'Vous avez besoin d’une meet-me-room neutre pour vous interconnecter sans dépendre d’un acteur dominant.' },
      { icon: 'target', titre: 'Partenaire pour l’expansion nationale', texte: 'Vous ne voulez pas signer avec 10 prestataires. Vous cherchez un partenaire avec un pipeline national et une gestion unifiée.' },
    ],
