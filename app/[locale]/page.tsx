...
export default async function Home({ params: { locale } }: { params: { locale: WpLocale } }) {
  ...
  // Diaporama du hero : les vues viennent de WordPress (image principale +
  // répéteur). Sans WordPress, l'image livrée avec le site fait office de vue
  // unique — le composant se replie alors sur une image simple, sans script.
  const heroSlides = wp?.heroSlides?.length
    ? wp.heroSlides
    : [{ sourceUrl: '/hero-datacenter.jpg', altText: t.heroImageAlt }];
  ...
  return (
    <main>
      ...
      {/* BANNIÈRE D'ACTUALITÉ — masquée tant qu'elle n'est pas activée dans WP */}
      {wp?.banner && <HomeBanner banner={wp.banner} locale={locale} />}
